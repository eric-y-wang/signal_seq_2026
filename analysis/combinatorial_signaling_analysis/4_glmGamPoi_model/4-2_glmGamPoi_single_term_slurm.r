...
# 1) Define Parameters ---------------------------------------------------
filter_cutoff <- 0.1 # Minimum fraction of cells expressing a gene to be used in test
exp_layer <- "counts" # Expression layer to use
ref_level <- "linker_linker" # reference level
# h5ad file path
h5ad_file <- "/data1/rudenska/EYW/SIG13/scanpy_outs/SIG13_doublets_DSB7.h5ad"
# make checkpoint directory
checkpoint_dir <- file.path(paste0("/scratch/rudenska/glmGamPoi_single_term_",filter_cutoff,"filter_checkpoints"))
if (!dir.exists(checkpoint_dir)) dir.create(checkpoint_dir, recursive = TRUE)
# set output directory
output_dir <- "/data1/rudenska/EYW/git_projects/SIG13/analysis_outs/glmGamPoi/glmGamPoi_single_term"
...
# identify unique conditions (excluding reference)
unique_calls <- obs %>%
  filter(ligand_call_DSB7 != ref_level) %>%
  pull(ligand_call_DSB7) %>%
  as.character() %>%
  unique()

# convert obs to data.table for faster subsetting
obs_dt <- as.data.table(obs)
combo_entries <- map(unique_calls, function(ligand_call) {
  # subset obs to include only the current ligand_call and reference
  group_pull <- c(ligand_call, ref_level)
  obs_sub <- obs_dt[ligand_call_DSB7 %in% group_pull]
  if (nrow(obs_sub) == 0) return(NULL)
  list(
    condition = ligand_call,
...
cat("Will run GLM for", length(entries_to_run), "conditions.\n")

# 7) Define worker function ----------------------------------
# This function will be serialized and run on the worker nodes
run_glm_for_entry <- function(entry, exp_mtx_path, filter_cutoff, ref_level) {
  ...
  model.df <- tibble(
    ligand = ifelse(entry$ligand_calls == ref_level, 0, 1),
  ...
  tryCatch({
    ...
    res_inter <- test_de(fit, contrast = `ligand`) %>%
      as_tibble() %>% mutate(condition = ligand_name)
    coef_tbl <- as_tibble(fit$Beta, rownames = "name") %>% mutate(condition = ligand_name)
  ...
  cat("ERROR in GLM fitting for", ligand_name, ": ", conditionMessage(e), "\n")
...
tryCatch({
  ...
  for (i in seq_along(entries_to_run)) {
    ...
    futures[[entry$condition]] <- future({
      result <- try({
        run_glm_for_entry(entry, exp_mtx_path, filter_cutoff, ref_level)
      }, silent = TRUE)
      ...
      # Return condition ID to indicate completion
      entry$condition
    })
    
    cat("Submitted job for combination:", entry$condition, "\n")
  }
...
if (!is.null(interaction_de_results)) {
  write_csv(interaction_de_results,
            file.path(output_dir, paste0("glmGamPoi_singleTerm_lfc_", filter_cutoff,"filter.csv")))
  write_csv(interaction_de_results %>% filter(adj_pval < 0.1),
            file.path(output_dir, paste0("glmGamPoi_singleTerm_lfc_sig_", filter_cutoff,"filter.csv")))
}
